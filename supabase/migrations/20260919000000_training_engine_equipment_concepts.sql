...
begin
  for e in select id, name from public.equipment where concepts is null loop
    ...
    if s ~ 'caminadora|cinta|trotadora' then c := c || array['cinta']; end if;
    if s ~ 'bicicleta|spinning' then c := c || array['bicicleta']; end if;
    ...
    elsif s ~ '\mbanco\M' then c := c || array['banco']; end if;
    if s ~ 'mancuerna' then c := c || array['mancuernas']; end if;
    if s ~ 'polea|cable|cruce' then c := c || array['polea']; end if;
    if s ~ 'prensa' then c := c || array['prensa']; end if;
    if s ~ 'hack' then c := c || array['maquina_hack']; end if;
    if s ~ 'asistid' then c := c || array['maquina_asistida','barra_fija']; end if;
    if s ~ 'predicador|scott' then c := c || array['banco_predicador']; end if;
    if s ~ 'dominad|paralel|fondos|barra fija|dip' then c := c || array['barra_fija']; end if;
    if s ~ 'kettlebell|pesa rusa' then c := c || array['kettlebell']; end if;
    if s ~ 'saco|boxeo|bolsa' then c := c || array['saco_boxeo','guantes']; end if;
    if s ~ 'caj[oó]n|plyo|\mbox\M' then c := c || array['cajon']; end if;
    if s ~ 'soga|battle' then c := c || array['battle_ropes']; end if;
    if s ~ 'bal[oó]n medicinal|medicine ball' then c := c || array['balon_medicinal']; end if;
    if s ~ 'barra ez|barra z' then c := c || array['barra_ez'];
    elsif s ~ '\mbarra\M' and not ('barra' = any(c)) then c := c || array['barra']; end if;
    if s ~ 'remo' and not ('polea' = any(c)) and not ('maquina' = any(c)) then c := c || array['remo_ergometro']; end if;
    if s ~ 'm[aá]quina' and not ('maquina_hack' = any(c)) and not ('maquina_asistida' = any(c)) then c := c || array['maquina']; end if;
