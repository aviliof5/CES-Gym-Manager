...
**Fase 15 — QR real + cámara.** ✅ Completa (2026-09-04). Cerraba el único gap que quedaba documentado desde la Fase 1: el código/link de invitación y "Mi QR" del cliente eran un cuadriculado de CSS puramente decorativo, y el check-in se hacía solo con un click manual del staff por cliente en la lista — nunca había cámara ni una librería de lectura de QR.

- **Generación real**: `qrcode-generator.min.js` (kazuhikoarase, MIT) vendorizado localmente (no CDN) en la raíz del repo — funciona offline en la PWA y dentro de la app empaquetada con Capacitor, igual que el resto de la app. `src/qr.js` (`drawQrCode`/`paintQrCodes`) dibuja códigos reales en un `<canvas>` nítido en pantallas retina. Reemplazó el cuadriculado decorativo en 3 lugares: código/link de invitación del dueño (`inviteCard()`), "Mi QR" del cliente (`qrCard()`, codifica `{t:'checkin', gym, u}`), y el QR de cobro en efectivo (codifica `{t:'payment', id}`, sin flujo de lectura propio — solo por consistencia visual, ya no queda ningún QR falso en la app).
- **Lectura real por cámara**: `jsQR.min.js` (cozmo/jsQR, MIT) también vendorizado. Pantalla nueva "Escanear QR" (`viewScanCheckin` en owner.js, botón junto a la lista de clientes) abre la cámara (`getUserMedia`, `facingMode:'environment'`) y lee QR en vivo (`ensureQrScanner`/`stopQrScanner` en `src/qr.js`) — apenas lee un código de check-in válido llama a `handleCheckinScan()` (actions.js), que hace las mismas validaciones honestas de siempre (payload bien formado, mismo gimnasio, cliente existe, no duplicar si ya tiene check-in hoy) y termina llamando exactamente al mismo `check_in_client()` que ya usaba el botón manual "Registrar entrada" — **la seguridad real sigue siendo 100% del RPC server-side** (`app_role_is_staff()`), leer el QR es solo un atajo de UX, nunca un camino nuevo de autorización. El botón manual se mantiene como alternativa (sin cámara a mano, o el cliente sin el código a la vista).
- **Manejo de errores real, no solo el camino feliz**: permiso de cámara denegado, sin cámara en el dispositivo, cámara ocupada por otra app — cada uno con su mensaje en español, sin crashear la pantalla (ver `cameraErrorMessage()` en router.js). Bug real encontrado y corregido en el camino: la primera versión reintentaba `getUserMedia()` en cada `render()` mientras hubiera error, lo que producía un loop infinito de reintentos (cada fallo disparaba `setState()`, que disparaba otro `render()`, que reintentaba) — corregido con un guard (`!state.scanError`) que solo reintenta cuando el usuario sale y vuelve a entrar a la pantalla, o toca "Reintentar" a mano. Verificado en una pestaña de navegador limpia contando errores de consola antes/después del fix (de ~50 a exactamente 1 por intento).
- **Fuera del alcance de esta fase, documentado como paso pendiente**: la app empaquetada con Capacitor (Android/iOS) necesita el permiso de cámara declarado en el proyecto nativo (`ces-android-build`/`ces-ios-build`, fuera de este repo) para que el WebView pueda pedirlo — instrucciones exactas agregadas en `ANDROID.md`/`IOS.md`. En el navegador/PWA funciona directo, sin ningún paso extra.
- **Verificado sin cámara física disponible** (el entorno de pruebas la bloquea a nivel de sandbox): el round-trip generación→lectura se probó directo (`drawQrCode` + `jsQR` sobre el mismo payload, coincide byte a byte), y la lógica completa de `handleCheckinScan()` se ejercitó llamándola directo con payloads reales (válido, de otro gimnasio, cliente ya registrado hoy, JSON corrupto) — los 4 casos responden exactamente lo esperado y el check-in real queda registrado en `todayCheckins`.
