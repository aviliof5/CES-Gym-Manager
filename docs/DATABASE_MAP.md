...
3. ✅ **Resuelto (2026-09-04)**: mínimo de "10 clientes interesados" para entrenador. Tabla `trainer_candidate_interest` (candidate_user_id, client_user_id, gym_id) + RPCs `mark_trainer_interest()`/`unmark_trainer_interest()` (el cliente nunca inserta/borra la fila directo). `approve_trainer()` ahora cuenta filas reales (`count(*) >= 10`) server-side antes de permitir `status → approved` — verificado que rechaza a los 9 y acepta al llegar a 10, sin importar qué muestre la UI. Migración: `20260904000200_trainer_interest.sql` — escrita, verificada contra `test-harness.html`, y **aplicada al proyecto Supabase real el 2026-09-04** (confirmado en vivo: tabla + ambas funciones RPC + política RLS existen, y `approve_trainer()` de verdad contiene la lógica de conteo nueva).
