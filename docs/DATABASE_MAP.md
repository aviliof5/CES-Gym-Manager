...
1. **Check-in por evento**: falta una tabla tipo `checkin_events (id, gym_id, client_user_id, created_at)` para un QR real escaneado por staff — `checkins` actual es un agregado, no sirve para "hoy a las 14:32 entró Fulano".
2. **Invitación de gimnasio por link/QR**: no existe columna `invite_code`/`invite_token` en `gyms`, ni endpoint/RPC que la valide. `join_gym(p_gym_id)` hoy solo exige que el gym exista — cualquier usuario autenticado puede unirse a cualquier `gym_id`, no requiere conocer un código.
