...
## Fase 17 (2026-09-20) — presencia en el gym: el cliente SÍ se autoacredita (cambio deliberado del modelo de amenazas)

`scan_gym_qr()` (ver `20260920000000_gym_presence_sessions.sql`) es la primera excepción a un principio que se venía siguiendo estricto desde la Fase 9/15: **hasta ahora, un cliente nunca podía registrar su propio check-in** — el QR era solo un identificador visual, siempre lo escaneaba el staff (`check_in_client()` exige `app_role_is_staff()`). El pedido de esta feature es justo lo contrario: un QR único y fijo del gimnasio, que tanto el staff (para quedar de "encargado de turno") como el propio cliente (para arrancar su sesión de 2 horas) escanean por su cuenta.

- **Qué cambia de verdad**: la rama `client` de `scan_gym_qr()` inserta una fila en `gym_sessions` a nombre de quien llama (`auth.uid()`), sin que ningún staff lo confirme. Como el QR sigue siendo texto plano no firmado (mismo criterio que todos los QR de esta app, ver Fase 15), cualquiera que fotografíe el cartel una sola vez puede "marcarse presente" repetidas veces sin estar físicamente en el gimnasio.
- **Por qué se acepta este riesgo**: lo único que depende de esa fila es un contador de ocupación en la pantalla del encargado y un recordatorio personal de 2 horas — no autoriza ningún cobro, no abre ninguna puerta física, y si un cliente se "hace trampa" a sí mismo (aparece como presente sin estarlo) no perjudica a nadie más que a la exactitud del contador. Es una decisión de producto, no un descuido — documentada acá para que quede explícita y no se confunda con las Fases 9/15, que sí exigían presencia real del staff.
- **Lo que SIGUE gateado server-side, sin excepción**: `scan_gym_qr()` deriva el gimnasio de `app_gym_id()`, nunca del texto del QR (mismo patrón que `check_in_client()`); la rama cliente exige `membership_status = 'al_dia'` — cierra un hueco que existía desde antes en el candado de "App bloqueada" de `viewClientHome` (ver el comentario en `src/screens/client.js`, "El check-in físico... es un tema aparte"), que hasta ahora era 100% de frontend; convertirse en encargado como entrenador exige `trainers.status = 'approved'` en ESE gimnasio, verificado dentro de la función.
- **`sync_gym_sessions()`** (recomputo perezoso del vencimiento, mismo criterio sin cron que `sync_my_membership_status()`) solo puede vencer las sesiones del propio gimnasio de quien llama, y un cliente solo puede vencer la suya — nunca la de otro.
