...
- **Escrito en disco, NO aplicado todavía** (al momento de esta auditoría — aplicada después, ver `MIGRATION_PLAN.md` Fase 0): `20260903000001_owner_role_2_logic.sql` — funciones (`app_role_is_staff`, `create_gym`/`join_gym` actualizados, `approve_admin`/`reject_admin`), políticas RLS nuevas, y la migración de datos que convierte la cuenta admin real (`fernandezavilio5@gmail.com`) en `owner`.
  - ⚠️ **Corrección (2026-09-05, ver `MIGRATION_PLAN.md` Fase 16)**: el email de esta nota resultó no coincidir con ninguna fila real de `profiles` en producción — el `role='owner'` real hoy es `bf825898@gmail.com`; `fernandezavilio5+cestest@gmail.com` (alias de prueba) es `role='admin'`. No se investigó a fondo el porqué del desfase (pudo cambiar el email de la cuenta entre esta auditoría y ahora, o el dato de esta nota nunca fue exacto) — se deja constancia para que una futura migración no repita el mismo supuesto sin verificar primero contra la base real.
